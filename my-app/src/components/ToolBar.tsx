...
export default function ToolBar({ activeTool, onToolChange, onUndo, onRedo, canUndo, canRedo, scale, onZoomIn, onZoomOut, showLongBeads = false, onToggleLongBeads }: ToolBarProps) {
  return (
    <div className="w-12 bg-gray-50 border-r border-gray-200 flex flex-col items-center py-2 gap-1">
      ...
      {onToggleLongBeads && (
        <button
          ...
          title="长条豆"
          className={`w-9 h-9 rounded-md flex items-center justify-center text-lg transition-colors ${showLongBeads ? 'bg-amber-100 text-amber-700' : 'hover:bg-gray-100'}`}
        >
          ➡️
        </button>
      )}
